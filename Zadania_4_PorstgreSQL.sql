...
*/
explain analyze -- Plannig time ~ 0.230 ; Execution Time ~ 1.6
with customers_films as(
select distinct
r.customer_id as Айди_покупателя,
count(f.film_id) over (partition by r.customer_id) as films_count
...
explain analyze -- Plannig time ~ 0.450 ; Execution Time ~ 1.8
select distinct
r.customer_id as Айди_покупателя,
count(f.film_id) over (partition by r.customer_id) as films_count
from film f
...
explain analyze -- Plannig time ~ 0.055 ; Execution Time ~ 0.04 
select
*
from SpecialView ; 
